...
function ProductCard({ p }) {
  return (
    <article className="group relative flex flex-col overflow-hidden rounded-2xl border border-bone/10 bg-bone/[0.02] transition hover:border-bone/30">
      <div className="relative aspect-[4/5] overflow-hidden bg-reef-deep">
        ...
        />
        {p.tag && (
          <span className="absolute left-3 top-3 rounded-full bg-reef-coral px-3 py-1 font-mono text-[10px] uppercase tracking-[0.15em] text-forest">
            {p.tag}
          </span>
        )}
      </div>
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <p className="mt-2 flex-1 text-sm leading-relaxed text-bone/55">{p.blurb}</p>

        <div className="mt-5 flex items-center justify-between">
          <div className="font-mono text-sm">
            <span className="text-bone">${p.price.toLocaleString()}</span>
            <span className="ml-2 text-bone/40">/ {p.unit}</span>
          </div>
          <button className="rounded-full border border-bone/20 px-4 py-2 font-mono text-[10px] uppercase tracking-[0.2em] transition group-hover:border-reef-glow group-hover:text-reef-glow">
            Add to tank
          </button>
        </div>
      </div>
...
export default function Store() {
  ...
  return (
    <section id="store" className="border-t border-bone/10 bg-gradient-to-b from-night to-ink">
